...
self.tinyvit.eval()
```
and the `pretrain_dinov3.yaml`
header ("DINOv3 ViT-S/16 (frozen, 21.6M)"). (The LIBERO language encoder is a 0.40M
learned encoder, trainable; the 4.4M bert-tiny used in the real-world final product
can be frozen via `language_encoder.py:85`.) The `vision.freeze=false` flag in the
...
VLA-DSS total               : 28.9M   (INCLUDES its own 0.40M learned text encoder)
=> deployable size ratio    : 4.7x
```
VLA-DSS breakdown (counted from checkpoint): vision 22.9M (DINOv3 21.6M frozen +
scattering) | language 0.40M (learned) | fusion 2.83M | FNO 2.43M | proprio+aux 0.04M
= 28.6M total, 7.03M trainable.
Note: VLA-DSS's 28.9M **already includes its 0.40M learned text encoder** -> fair
comparison is 28.9M (incl. lang) vs Octo's 136.7M (incl. its 109.6M T5), not vs
Octo's bare 27M. (A separate ~4M bert-tiny is used for real-world only, NOT in 28.9M.)
