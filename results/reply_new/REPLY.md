...
| **VLA-DSS (LIBERO experiments)** | **28.9M** (0.40M learned text enc) |
| **VLA-DSS FINAL PRODUCT** | **~32.9M** (4.4M bert-tiny text enc) |

**The final product / deployed model is the +4.4M bert-tiny version (~32.9M total)**
— bert-tiny gives robust real-world language grounding. LIBERO ablations used the
lighter 0.40M learned encoder (28.9M). Octo's "27M" is only its action transformer;
it cannot read an instruction without the frozen 109.6M T5. Either VLA-DSS variant
already includes its own text encoder, so the fair comparison is ~29-33M (incl.
lang) vs 136.7M (incl. Octo's 109.6M T5) -- NOT vs Octo's bare 27M. VLA-DSS does
language in 0.40-4.4M vs Octo's 109.6M. Efficiency claim: **~4x fewer deployable
params** — and on trainable params, **7.03M vs Octo's ~27M**.
