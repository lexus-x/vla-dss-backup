# Live monitor: LIBERO-Spatial+aux ladder eval (ep5/10/15/20) -> find the success peak.
$host.UI.RawUI.WindowTitle = "Spatial+aux eval"
while ($true) {
  Clear-Host
  Write-Host "=== LIBERO-Spatial+aux ladder (find success peak)   $(Get-Date -Format 'HH:mm:ss') ===" -ForegroundColor Cyan
  Write-Host "  ref: Object aux 71% / DAgger 79.5% | my Spatial guess ~74%`n" -ForegroundColor DarkGray
  Write-Host ("  {0,-7} {1,-16} {2,-9} {3}" -f "ckpt","success","jerk","prog") -ForegroundColor Yellow
  foreach($ep in 5,10,15,20){
    $f="E:\fno_data\zspa_$ep.jsonl"
    if(Test-Path $f){
      ...
      if($r){ $n=@($r).Count;$s=(@($r)|Measure-Object success -Sum).Sum;$sr=[math]::Round(100*$s/$n,1)
        ...
        $col=if($sr -ge 74){"Green"}elseif($sr -ge 65){"Yellow"}else{"White"}
  ...
  $latest=$null
  foreach($ep in 20,15,10,5){ if(Test-Path "E:\fno_data\zspa_$ep.jsonl"){ $rr=Get-Content "E:\fno_data\zspa_$ep.jsonl" -EA SilentlyContinue|Where-Object{$_}; if(@($rr).Count -gt 0){ $latest=$ep; break } } }
  if($latest){
    $r=Get-Content "E:\fno_data\zspa_$latest.jsonl"|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json}
    ...
    foreach($ti in 0..9){
      ...
      if($g){ $n=@($g).Count;$s=(@($g)|Measure-Object success -Sum).Sum
        $nm=($g[0].task -replace 'pick up the ','' -replace ' and place it on the plate','')
        if($nm.Length -gt 36){$nm=$nm.Substring(0,36)}
