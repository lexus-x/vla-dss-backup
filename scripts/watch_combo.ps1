...
$f = "c:\sarvik\fno_backup\logs\_train_combo.txt"
$host.UI.RawUI.WindowTitle = "TRAIN combo"
function Bar($pct,$w){ $fl=[int]($pct/100*$w); ("#"*$fl)+("-"*($w-$fl)) }
while ($true) {
  Clear-Host
  Write-Host "=== COMBO (gate+aug+DAgger)   $(Get-Date -Format 'HH:mm:ss') ===" -ForegroundColor Cyan
  Write-Host "  ref: vs aux 71% / DAgger 79.5% (this adds gate+robustness)`n" -ForegroundColor DarkGray
