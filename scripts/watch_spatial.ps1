# Training monitor for LIBERO-Spatial finetune.
$f = "c:\sarvik\fno_backup\logs\_train_spatial_aux.txt"
$host.UI.RawUI.WindowTitle = "TRAIN spatial+aux"
function Bar($pct,$w){ $fl=[int]($pct/100*$w); ("#"*$fl)+("-"*($w-$fl)) }
while ($true) {
  Clear-Host
  Write-Host "=== LIBERO-Spatial finetune   $(Get-Date -Format 'HH:mm:ss') ===" -ForegroundColor Cyan
  ...
  if($rows.Count){
    ...
    Write-Host ("`n  {0,-7} {1,-10} {2,-10} {3}" -f "epoch","train","val(ema)","") -ForegroundColor Yellow
    ...
    Write-Host ("`n  BEST val(ema) {0:N4}   ({1} epochs)   [early-stop ~ep5]" -f $best,$rows.Count) -ForegroundColor Cyan
