...
$f = "c:\sarvik\fno_backup\logs\_train_goal_aux.txt"
$host.UI.RawUI.WindowTitle = "TRAIN goal+aux"
function Bar($pct,$w){ $fl=[int]($pct/100*$w); ("#"*$fl)+("-"*($w-$fl)) }
while ($true) {
  Clear-Host
  Write-Host "=== LIBERO-Goal+aux finetune (max 30ep)   $(Get-Date -Format 'HH:mm:ss') ===" -ForegroundColor Cyan
  Write-Host "  ref: Object aux 71% | Spatial aux 73% | guess Goal ~70-75%`n" -ForegroundColor DarkGray
