# Live monitor: LIBERO-Long+aux ladder eval (ep5/10/15/20) -> find success peak. 4th suite.
$host.UI.RawUI.WindowTitle = "Long+aux eval"
while ($true) {
  Clear-Host
  Write-Host "=== LIBERO-Long+aux ladder (HARD suite)   $(Get-Date -Format 'HH:mm:ss') ===" -ForegroundColor Cyan
  Write-Host "  ref: Object 71 / Spatial 73 / Goal 72 | Long is long-horizon -> expect ~40-55%`n" -ForegroundColor DarkGray
  Write-Host ("  {0,-7} {1,-16} {2,-9} {3}" -f "ckpt","success","jerk","prog") -ForegroundColor Yellow
  foreach($ep in 5,10,15,20){
    $f="E:\fno_data\zlng_$ep.jsonl"
    if(Test-Path $f){
      ...
      if($r){ $n=@($r).Count;$s=(@($r)|Measure-Object success -Sum).Sum;$sr=[math]::Round(100*$s/$n,1)
        ...
        $col=if($sr -ge 55){"Green"}elseif($sr -ge 40){"Yellow"}else{"White"}
  ...
  $latest=$null
  foreach($ep in 20,15,10,5){ if(Test-Path "E:\fno_data\zlng_$ep.jsonl"){ $rr=Get-Content "E:\fno_data\zlng_$ep.jsonl" -EA SilentlyContinue|Where-Object{$_}; if(@($rr).Count -gt 0){ $latest=$ep; break } } }
  if($latest){
    $r=Get-Content "E:\fno_data\zlng_$latest.jsonl"|Where-Object{$_}|ForEach-Object{$_|ConvertFrom-Json}
    ...
    foreach($ti in 0..9){
      ...
      if($g){ $n=@($g).Count;$s=(@($g)|Measure-Object success -Sum).Sum
        $nm=$g[0].task; if($nm.Length -gt 42){$nm=$nm.Substring(0,42)}
        $c=if(100*$s/$n -ge 60){"Green"}elseif(100*$s/$n -ge 30){"Gray"}else{"Red"}
